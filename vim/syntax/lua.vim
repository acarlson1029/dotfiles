...
" Support for Function Name Highlighting
syn match luaCustomParen "(" contains=luaParen
syn match luaCustomFunc  "[a-zA-Z0-9_.]\+\s*(" contains=luaCustomParen
hi def luaCustomFunc ctermfg=Blue
...
" Header Blocks
syn region luaHeaderBlock fold start="^-- [a-zA-Z0-9 :$_\-',=;\./?()]\+\n-\{60\}$" end="^\n-\{60\}" contains=ALLBUT,luaHeaderBlock keepend
syn region luaSubHeaderBlock fold start="^---- [a-zA-Z0-9 $\-_':,=;\./?()]\+\n-\{58\}" end="^\n-\{58\}" contains=ALLBUT,luaHeaderBlock keepend
...
syn region luaFunctionBlock fold transparent matchgroup=luaFunction start="function" end="\<end\>" contains=ALLBUT,luaTodo,luaSpecial,luaCond,luaCondElseif,luaCondEnd,luaRepeat

" if then else elseif end
syn keyword luaCond contained else

" then ... end
syn region luaCondEnd fold contained transparent matchgroup=luaCond start="\<then\>" end="\<end\>" contains=ALLBUT,luaTodo,luaSpecial,luaRepeat
...
" do ... end
syn region luaBlock fold transparent matchgroup=luaStatement start="\<do\>" end="\<end\>" contains=ALLBUT,luaTodo,luaSpecial,luaCond,luaCondElseif,luaCondEnd,luaRepeat

" repeat ... until
syn region luaRepeatBlock fold transparent matchgroup=luaRepeat start="\<repeat\>" end="\<until\>" contains=ALLBUT,luaTodo,luaSpecial,luaCond,luaCondElseif,luaCondEnd,luaRepeat
