...
plugins=(git python osx p4 tmux brew)
...
# Grab the shared aliases between zsh and bash
source $HOME/.alias
source $HOME/.private_alias

# Z support
# Let's you cd places in shorthand.
# Instead of doing cd ~/Somefolder/Anotherfolder/project/, you can just do 
# > z proj 
# It's that simple! Keep in mind that z is not a magic script that knows 
# where every folder on your computer is. It keeps track of your cd to 
# directories in a file called ~/.z - Once you've cd'd into a directory 
# once, you can use z to navigate to it.
